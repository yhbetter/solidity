+(function($, window, document, undefined){
    ...
    Checks.prototype = {
        ...
        search : function (sparam) {
            ...
            $grid.datagrid("datagrid")._params.status = this.isNotBlank(sparam.status)?sparam.status:null;
            $grid.datagrid("datagrid")._params.name = this.isNotBlank(sparam.name)?sparam.name:"";
            $grid.datagrid("datagrid")._params.category = this.isNotBlank(sparam.category)?sparam.category:"";
            $grid.datagrid("datagrid")._params.platformId = this.isNotBlank(sparam.platformId)?sparam.platformId:"";
            ...
            $(".btn-examine").on({
                click: function(){
                    ...
                    if ( fn === 'examine' ){
                        ...
                        $.post('examine',{id:sId,name:sName,checkedStatus:options.status.ignored,category:'NETWORK_MOVIE',_csrf:csrf},function(response){
        ...
        sDetail : function (bid,sid,sname,status,fn) {
            ...
            $.getJSON('detail/'+sname,{"id":sid},function(respData){
                ...
                $.each(data,function(j,s){
                    ...
                    $.each(data[j],function(k,v){
                        ...
                        table.push('        <td class="'+this_show_class+'"><a href="'+v.jobs[0].url+'" target="_Blank">'+v.name+'</a></td>');
                        table.push('    </tr>');
                        flag++;
                    });
                });
...
_default.grid.column = [{
    ...
    render: function (data) {
        var html = '',
            row = data.row,
            _atg = 'sId="'+row.id+'" sName="'+row.name+'" status="'+row.checkedStatus+'"';
