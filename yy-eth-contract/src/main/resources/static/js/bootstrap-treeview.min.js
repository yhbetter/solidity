    ...
    g.prototype.init = function (b) {
        ...
        if (b) {
            ...
            a.each(b, function (b, e) {
                ...
                var h = [];
                if (e.nodes.length > 0 && e.nodes ? (h.push("expand-icon"), h.push(e.state.expanded ? d.options.collapseIcon : d.options.expandIcon)) : h.push(d.options.emptyIcon), f.append(a(d.template.icon).addClass(h.join(" "))), d.options.showIcon) {
                    var h = ["node-icon"];
                ...
                if (d.options.showCheckbox && e.nodes.length == 0) {
                    var h = ["check-icon"];
